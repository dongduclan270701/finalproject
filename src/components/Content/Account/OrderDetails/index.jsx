...
const Index = () => {
    ...
    return (
        <>
            <div className="col-sm-9" style={{ borderRadius: "15px", background: "rgb(247 247 247)", fontSize: "14px" }}>
                <div className="row" style={{ padding: "20px", fontSize: "15px", borderBottom: "1px solid #e5dfdf" }}>
                ...
                <div className="row" style={{ height: "50px", padding: "0 15px", alignItems: "center", borderBottom: "1px solid rgb(254 240 191)", backgroundColor: "#f7f7d8" }}>
                ...
                {order && order.status === 'Delivery successful' && order.statusReview.status === false && <div className="row" style={{
                    height: "50px", padding: "0 15px", alignItems: "center", backgroundColor: "#f7f7d8"
                }}>
                ...
                {order && order.status === 'Delivery successful' && order.statusReview.status === true && <div className="row" style={{
                    ...
                    <div className="col-4" style={{ display: "flex", flexDirection: "row-reverse" }}>
                        <button className="btn btn-secondary btn-sm" onClick={handleToggleShowRate}>Review</button>
                    </div>
                </div>
                }
                <hr style={{ backgroundImage: "repeating-linear-gradient(45deg,#6fa6d6,#6fa6d6 33px,transparent 0,transparent 41px,#f18d9b 0,#f18d9b 74px,transparent 0,transparent 82px)", height: "5px" }} />
                <div className='row' style={{ padding: "15px 0px" }}>
                    ...
                    <div className="col-lg-6" style={{ borderLeft: "1px solid #cdcccc" }}>
                                    ...
                                    item.content === "Ordered" ? Process :
                                        item.content === "Payment information confirmed" ? Process :
                                            item.content === "Being transported" ? Delivery :
                                                item.content === "Delivered to the carrier" ? Delivery : Correct} style={{ width: "20px", padding: "0" }} />
                                <p style={{ paddingLeft: "15px", margin: 0, width: "unset" }}>{item.time} - {item.date} - {item.content}</p>
                            </div>
                        ))}
                    </div>
                </div>
                ...
                {order && order.product.map((item, index) => {
                    return <div style={{ borderBottom: "1px solid #e1dfdf" }} key={index}>
                        <div className="row" style={{ margin: "0px", padding: "10px 0px" }}>
                            ...
                            <div className="col-4" style={{ textAlign: "right", margin: "auto", color: "red" }}>₫ {formatter.format(item.quantity * item.nowPrice)}</div>
                        </div>
                    </div>
                })}
                <div className="row" style={{ padding: "0", alignItems: "center", borderBottom: "1px solid #e1dfdf" }}>
                ...
                <div className="row" style={{ height: "50px", padding: "0 15px", alignItems: "center", borderBottom: "1px solid #e1dfdf", backgroundColor: "#f7f7d8" }}>
                    <img src={logoWarning} alt="" style={{ width: "20px", padding: "0" }} />
                    <p style={{ margin: "0" }}>
                        Please pay ₫ <span style={{ color: '#fa577a' }}>{order && formatter.format(order.sumOrder + order.ship - sumDiscountListProduct())}</span> upon receipt.
                    </p>
                </div>
